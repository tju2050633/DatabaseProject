...
|      | account_name     | varchar(20)  | 用户名           | not null                                          |
...
| 序号 | 字段名        | 数据类型 | 说明       | 备注                                  |
| ---- | ------------- | -------- | ---------- | ------------------------------------- |
|      | login_info_id | char(10) | 登录信息号 | PK, not null                          |
|      | account_id    | char(10) | 账号       | FK from Account(account_id), not null |
|      | login_time    | date     | 登录时间   | not null                              |
...
|      | applier_id | char(10) | 申请者ID | FK from Account(account_id), not null |
...
|      | volunteer_id            | char(10) | 维护者ID | FK from Account(account_id), not null |
|      | maintenance_description | clob     | 维护描述 |                                       |
...
|      | activity_picture     | varchar(255) | 活动照片 | 存图床链接                            |
|      | activity_description | clob         | 活动描述 |                                       |
